...
const AccessibilityPage = () => {
    ...
    const { settings, updateSetting, decreaseFontSize, increaseFontSize, fontSize } = useAccessibility();
    const { theme, setTheme } = useTheme();
    const navigate = useNavigate();

    const [saving, setSaving] = useState(false);

    // Some local state for things not yet in AccessibilityContext (like TTS voice/speed demo)
    ...
    const handleSave = async () => {
        setSaving(true);
        try {
            // Context automatically syncs to backend after 1s, but we can do an explicit sync or just a toast
    ...
    const handleReset = () => {
        if (window.confirm('Reset all accessibility settings to default?')) {
            ...
            updateSetting('sttEnabled', false);
    ...
    return (
        <div className="flex min-h-screen w-full bg-background-light dark:bg-background-dark text-text-main dark:text-white font-display overflow-hidden transition-colors duration-300">
            {/* Side Navigation */}
                ...
                <div className="p-6 flex flex-col h-full justify-between">
                    <div className="flex flex-col gap-8">
                        ...
                        <div className="flex gap-4 items-center p-3 rounded-xl bg-background-light dark:bg-gray-800/50">
                            <Avatar src={user?.avatar} name={displayName} username={user?.username} userId={user?.id} size="size-12" />
                            <div className="flex flex-col overflow-hidden">
                                <h1 className="text-text-main dark:text-white text-base font-bold leading-tight truncate">{displayName}</h1>
                                <p className="text-primary text-xs font-semibold uppercase tracking-wider">{user?.is_online ? 'Online' : 'Offline'}</p>
                            </div>
                        </div>
                        {/* Navigation Links */}
                        <nav className="flex flex-col gap-2">
                            <Link to="/dashboard" className="flex items-center gap-4 px-4 py-3 rounded-xl text-text-main dark:text-gray-300 hover:bg-background-light dark:hover:bg-gray-800 transition-colors group">
                                <span className="material-symbols-outlined text-[24px] text-gray-500 group-hover:text-primary transition-colors" aria-hidden="true">home</span>
                            ...
                            <Link to="/chat" className="flex items-center gap-4 px-4 py-3 rounded-xl text-text-main dark:text-gray-300 hover:bg-background-light dark:hover:bg-gray-800 transition-colors group">
                                <span className="material-symbols-outlined text-[24px] text-gray-500 group-hover:text-primary transition-colors" aria-hidden="true">chat_bubble</span>
                                <span className="text-sm font-semibold">Messages</span>
                            </Link>
                            <Link to="/contacts" className="flex items-center gap-4 px-4 py-3 rounded-xl text-text-main dark:text-gray-300 hover:bg-background-light dark:hover:bg-gray-800 transition-colors group">
                                <span className="material-symbols-outlined text-[24px] text-gray-500 group-hover:text-primary transition-colors" aria-hidden="true">groups</span>
                                <span className="text-sm font-semibold">Contacts</span>
                            </Link>
                            {/* Active State */}
                            <Link to="/accessibility" aria-current="page" className="flex items-center gap-4 px-4 py-3 rounded-xl bg-primary/10 text-primary dark:text-primary transition-colors">
                                <span className="material-symbols-outlined text-[24px] fill-1" aria-hidden="true">settings_accessibility</span>
                            ...
                            <Link to="/profile" className="flex items-center gap-4 px-4 py-3 rounded-xl text-text-main dark:text-gray-300 hover:bg-background-light dark:hover:bg-gray-800 transition-colors group">
                                <span className="material-symbols-outlined text-[24px] text-gray-500 group-hover:text-primary transition-colors" aria-hidden="true">person</span>
                                <span className="text-sm font-semibold">Profile</span>
                            </Link>
                        </nav>
                    </div>
                    <div className="px-2">
                        <button onClick={handleLogout} className="flex items-center justify-center gap-2 w-full py-3 rounded-xl text-[#4596a1] hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors text-sm font-medium">
            ...
            <main className="flex-1 flex flex-col h-full overflow-hidden relative">
                ...
                <header className="flex-shrink-0 flex items-center justify-between border-b border-[#e6f3f4] dark:border-gray-800 bg-white dark:bg-gray-900 px-6 py-4 z-10 transition-colors duration-300">
                    <div className="flex items-center gap-4">
                        <Link to="/dashboard" className="lg:hidden p-2 -ml-2 text-gray-500 hover:text-primary">
                    ...
                    <div className="flex gap-3">
                        <button onClick={handleReset} className="hidden sm:flex h-10 px-6 items-center justify-center rounded-xl bg-background-light dark:bg-gray-800 text-text-main dark:text-gray-200 text-sm font-bold hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">
                            Reset
                        </button>
                        <button onClick={handleSave} disabled={saving} className="flex h-10 px-6 items-center justify-center gap-2 rounded-xl bg-primary text-white text-sm font-bold shadow-lg shadow-primary/30 hover:bg-[#006c7a] transition-all active:scale-95 disabled:opacity-70">
                            {saving ? <span className="material-symbols-outlined animate-spin" aria-hidden="true">progress_activity</span> : null}
                            {saving ? 'Saving...' : 'Save'}
                        </button>
                    </div>
                </header>
                        ...
                        <div className="xl:col-span-8 flex flex-col gap-8">
                            ...
                            <section className="flex flex-col gap-4">
                                ...
                                <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 border border-[#e6f3f4] dark:border-gray-800 shadow-sm transition-colors duration-300">
                                    ...
                                    <div className="mb-8">
                                        ...
                                        <div className="flex items-center gap-4">
                                            <span className="text-xs font-medium text-gray-500">Aa</span>
                                            <input
                                        ...
                                        {[
                                            ...
                                            <label key={filter.id} className="cursor-pointer group">
                                                <input
                                                    ...
                                                    checked={settings.colorBlindMode === filter.id}
                                                    onChange={() => updateSetting('colorBlindMode', filter.id)}
                                                    aria-label={`Color filter: ${filter.label}`}
                                                />
                                ...
                                <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 border border-[#e6f3f4] dark:border-gray-800 shadow-sm transition-colors duration-300">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <h4 className="text-sm font-bold text-text-main dark:text-white">Reduce Motion</h4>
                                            <p className="text-xs text-gray-500 mt-1">Minimize animations and UI transitions</p>
                                        </div>
                                        <label className="relative inline-flex items-center cursor-pointer">
                                            <input
                                                type="checkbox"
                            ...
                            <section className="flex flex-col gap-4">
                                ...
                                <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 border border-[#e6f3f4] dark:border-gray-800 shadow-sm transition-colors duration-300">
                                    ...
                                    <div className="flex items-center justify-between mb-6 pb-6 border-b border-[#e6f3f4] dark:border-gray-800">
                                        <div>
                                            <h4 className="text-sm font-bold text-text-main dark:text-white">Enable TTS</h4>
                                            <p className="text-xs text-gray-500 mt-1">Read text aloud throughout the application</p>
                                        </div>
                                        <label className="relative inline-flex items-center cursor-pointer">
                                            <input
                                                type="checkbox"
                                    ...
                                    <div className={`flex flex-col md:flex-row gap-8 ${!settings.ttsEnabled ? 'opacity-50 pointer-events-none' : ''}`}>
                                        {/* Voice Selection */}
                                        <div className="flex-1">
                                            ...
                                            <div className="relative">
                                                ...
                                                <span className="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none" aria-hidden="true">expand_more</span>
                                            </div>
                            ...
                            <div className="sticky top-4 space-y-4">
                                    ...
                                    <div className="relative z-10 flex flex-col gap-4 transition-all duration-300" style={{ fontSize: `${fontSize}px` }}>
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="space-y-1">
                                                <div className={`h-3 w-24 bg-gray-200 dark:bg-gray-700 rounded ${settings.reduceMotion ? '' : 'animate-pulse'}`}></div>
                                                <div className="h-2 w-16 bg-gray-100 dark:bg-gray-800 rounded"></div>
                                            </div>
                                        </div>
                                        <div className="space-y-2">
                                            <div className={`h-4 w-3/4 bg-gray-200 dark:bg-gray-700 rounded ${settings.reduceMotion ? '' : 'animate-pulse'}`}></div>
                                            <div className={`h-4 w-full bg-gray-200 dark:bg-gray-700 rounded ${settings.reduceMotion ? '' : 'animate-pulse'}`}></div>
                                            <div className={`h-4 w-5/6 bg-gray-200 dark:bg-gray-700 rounded ${settings.reduceMotion ? '' : 'animate-pulse'}`}></div>
                                        </div>
                                ...
                                <div className="bg-primary text-white rounded-2xl p-6 shadow-xl relative overflow-hidden">
                                    <div className="relative z-10">
                                        ...
                                        <ul className="space-y-3 text-sm font-medium">
                                            <li className="flex items-center gap-3 opacity-90">
                                                <span className="material-symbols-outlined text-[20px]" aria-hidden="true">text_increase</span>
                                                <span>Large Text {fontSize > 16 ? 'enabled' : 'disabled'}</span>
                                            </li>
                                            <li className="flex items-center gap-3 opacity-90">
                                                <span className="material-symbols-outlined text-[20px]" aria-hidden="true">contrast</span>
                                    ...
                                    <div className="absolute top-0 right-0 p-8 opacity-10">
                                        <span className="material-symbols-outlined text-9xl" aria-hidden="true">accessibility</span>
                                    </div>
                                </div>
                            </div>
                        </div>
